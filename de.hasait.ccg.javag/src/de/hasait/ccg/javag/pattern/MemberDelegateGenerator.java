/*
 * $Id: MemberDelegateGenerator.java,v 1.1 2005-10-02 00:34:30 a-pi Exp $
 * 
 ...
 */
public class MemberDelegateGenerator implements ICcgGenerator {
    private final String[] KEYWORDS = new String[] { "JavaMemberDelegate" };

    public String getDescription() {
        return "Member delegate pattern generator - adds public method delegates";
    }
    ...
    public String generate(final Element element,
        ...
        while (types.hasNext()) {
            ...
            if (abstractTypeDeclaration instanceof TypeDeclaration) {
                ...
                for (int f = 0; f < fieldDeclarations.length; f++) {
                    ...
                    variableDeclarationFragments = fieldDeclaration.fragments()
                            .iterator();
                    while (variableDeclarationFragments.hasNext()) {
                        variableDeclarationFragment = (VariableDeclarationFragment) variableDeclarationFragments
                                .next();
                        if (variableDeclarationFragment.getName()
                                .getFullyQualifiedName().equals(name)) {
                            StringWriter result = new StringWriter();
                            PrintWriter presult = new PrintWriter(result);
                            presult.println();
                            /*
                             * presult .println("\t//delegation for " + name +
                             * "(commentpos:" + ccgComment.getBeginLine() + "#" +
                             * ccgComment.getBeginColumn() + "@vardeclpos:" +
                             * compilationUnit
                             * .lineNumber(variableDeclarationFragment
                             * .getStartPosition()) + ")");
                             */
                            ...
                            typeBinding = fieldDeclaration.getType()
                                    .resolveBinding();
                            methodBindings = typeBinding.getDeclaredMethods();
                            for (int m = 0; m < methodBindings.length; m++) {
                                methodBinding = methodBindings[m];
                                if (Modifier.isPublic(methodBinding
                                        .getModifiers())
                                        && !methodBinding.isConstructor()) {
                                    method = (IMethod) methodBinding
                                            .getJavaElement();
                                    parameterTypes = methodBinding
                                            .getParameterTypes();
                                    parameterNames = method.getParameterNames();
                                    presult.print("\t/** Delegate to {@link "
                                            + typeBinding.getQualifiedName()
                                            + "#" + methodBinding.getName()
                                            + "(");
                                    for (int p = 0; p < parameterTypes.length; p++) {
                                        presult.print(parameterTypes[p]
                                                .getQualifiedName());
                                    ...
                                    presult.println(") {");
                                    presult.print("\t\t");
                                    if (!(returnType.isPrimitive() && returnType
                                            .getName().equals("void"))) {
        ...
        throw new IllegalArgumentException("Invalid or undeclared member "
                + name);
    }
}
